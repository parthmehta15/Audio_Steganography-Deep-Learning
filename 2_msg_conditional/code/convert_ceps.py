...
def convert(solver, carrier_wav_path, msg_wav_paths, trg_dir, epoch, trim_start, num_samples):
    ...
    out_carrier = stft.inverse(spect_carrier_reconst).squeeze(0).detach().numpy()
    orig_out_carrier = stft.inverse(carrier.cpu()).squeeze(0).detach().numpy()

    outs_msg = [stft.inverse(spect_msg_reconst).squeeze(0).detach().numpy() for spect_msg_reconst in spects_msg_reconst]
    orig_outs_msg = [stft.inverse(spect_msg.cpu()).squeeze(0).detach().numpy() for spect_msg, phase_msg in msg]
