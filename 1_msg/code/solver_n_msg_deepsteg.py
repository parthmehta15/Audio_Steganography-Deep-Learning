...
class SolverNMsgMultipleDecodersDeepSteg(Solver):
    def __init__(self, config):
        ...
        # ------ create models ------
        self.dec_c_conv_dim = 2*self.n_messages + 2 + 64
        self.build_models()
    ...
    def forward(self, carrier, msg):
        ...
        # concat all msg_i into single tensor
        merged_enc = torch.cat((carrier_enc, carrier, msg), dim=1)  # concat encodings on features axis
        ...
        '''if (self.add_stft_noise != -1 and self.cur_iter > self.add_stft_noise) or self.mode == 'test':
            ...
            carrier_reconst_tag, _ = self.stft.transform(y)
        ...
        msg_reconst_list = torch.cat(msg_reconst_list, dim=1)
