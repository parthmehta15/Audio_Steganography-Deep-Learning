...
class Solver(object):
    ...
    def train(self):
        ...
        for epoch in epoch_it:
            ...
            # inner epoch loop
            for carrier, msg in it:

                carrier = carrier.squeeze(1)
                msg = [i.squeeze(1) for i in msg]

                self.cur_iter += 1
                i = self.cur_iter
                batch_size, channel, h, w = carrier.shape

                # feedforward and suffer loss
                carrier_reconst, msg_reconst = self.forward(carrier, msg)
    ...
    def test(self, data='test'):
        ...
        with torch.no_grad():
            ...
            logger.info(f"Mode: {'test' if data == 'test' else 'validation'}")
            data = self.test_loader if data == 'test' else self.val_loader
            # start of training loop
            logger.info("start testing...")
            for carrier, msg in tqdm(data):
    ...
    def sample_examples(self, n_examples=5, subdir=None):
        if self.mode != 'test':
