...
class SolverNMsgCond(Solver):
    def __init__(self, config):
        ...
        # ------ create models ------
        self.dec_c_conv_dim = self.n_messages * (self.n_messages+2) + 2 + 64
    ...
    def build_models(self):
        ...
        self.enc_m = Encoder(conv_dim   = 2 + self.n_messages,
                             block_type = self.block_type,
                             n_layers   = self.config.enc_n_layers)

        self.enc_c = Encoder(conv_dim   = 2,
                             block_type = self.block_type,
                             n_layers   = self.config.enc_n_layers)
    ...
    def forward(self, carrier, msg):
        ...
        # print('Carrier_shape: ',carrier.shape )

        # encode carrier
        carrier_enc = self.enc_c(carrier)
        # print('Ecoded_carrier: ',carrier_enc.shape)
        ...
        for i in range(self.n_messages):
            ...
            #     print('Message_reconst_shape: ',msg_reconst.shape)
        # print('#######################')
