...
def spect_loader(path, trim_start,  num_samples=16000, crop=True):
...
class BaseLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        carrier_real, carrier_img = self.loader(carrier_file,
                                                   self.trim_start,
                                                #    return_phase=True,
                                                   num_samples=self.num_samples)
        ...
        msg_real = list(map(lambda x: x[0], msg))
        msg_img = list(map(lambda x: x[1], msg))
        ...
        return carrier_real, carrier_img, msg_real, msg_img
