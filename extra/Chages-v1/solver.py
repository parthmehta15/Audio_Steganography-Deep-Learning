...
class Solver(object):
    ...
    def train(self):
        ...
        for epoch in epoch_it:
            ...
            for carrier_real, carrier_imag, msg_real, msg_imag in it:
                self.cur_iter += 1
                i = self.cur_iter
                carrier = torch.cat(carrier_real, carrier_imag, dim =1)
                batch_size, _, h, w = carrier.shape

                msg = torch.cat(msg_real, msg_imag, dim =1)
    ...
    def test(self, data='test'):
        ...
        with torch.no_grad():
            ...
            # for carrier_real, carrier_imag, msg, in tqdm(data):
            for carrier_real, carrier_imag, msg_real, msg_imag, in tqdm(data):
