...
class SolverNMsgMultipleDecodersDeepSteg(Solver):
    def __init__(self, config):
        ...
        # ------ create models ------
        self.dec_c_conv_dim = self.n_messages*2 + 2 + 64
        self.build_models()
    ...
    def forward(self, carrier, msg):
        ...
        if (self.add_stft_noise != -1 and self.cur_iter > self.add_stft_noise) or self.mode == 'test':
            ...
            # print('Input to inverse  STFT: ', carrier_reconst.shape)
            y = self.stft.inverse(carrier_reconst)
            # print('output from inverse  STFT: ', y.shape)
            real_carrier_reconst_tag, imag_carrier_reconst_tag = self.stft.transform(y)

            real_carrier_reconst_tag = real_carrier_reconst_tag.unsqueeze(1)
            imag_carrier_reconst_tag = imag_carrier_reconst_tag.unsqueeze(1)

            carrier_reconst_tag = torch.cat((real_carrier_reconst_tag, imag_carrier_reconst_tag),dim =1)

            # print('output form stft: ',carrier_reconst_tag.shape)
            self.stft.to('cpu')
        else:
        ...
        for i in range(len(msg)):  # decode each msg_i using decoder_m_i
            ...
            # print('Reconstructed_msg: ',msg_reconst.shape)
