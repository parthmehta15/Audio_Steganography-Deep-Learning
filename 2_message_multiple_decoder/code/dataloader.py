...
def spect_loader(path, trim_start, return_phase=False, num_samples=16000, crop=True):
    ...
    real_part,imag_part = stft.transform(y)
    # print('real_stft ',real_part.shape)
    # print('imag_stft ',real_part.shape)

    real_part = real_part.unsqueeze(1) #addinng channel dim (B,C,X,Y)
    imag_part = imag_part.unsqueeze(1)

    # print('real_stft_channel ',real_part.shape)

    fft_audio = torch.cat((real_part, imag_part),dim =1)

    # print('stft_Carrier_shape: ', fft_audio.shape)
...
class BaseLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        carrier = self.loader(carrier_file, self.trim_start, num_samples=self.num_samples)
